...
static const char *const TAG = "a76xx";
...
void A76XXComponent::send_cmd_(const std::string &message) {
...
void A76XXComponent::parse_cmd_(std::string message) {
  if (message.empty())
    return;
  ...
  } else if (this->state_ == STATE_SETUP_CPMS) {
  ...
  switch (this->state_) {
    ...
    case STATE_SETUP_CMGF:
      send_cmd_("AT+CMGF=1");
      this->state_ = STATE_SETUP_CPMS;
      this->expect_ack_ = true;
      break;
    case STATE_SETUP_CPMS:
      send_cmd_("AT+CPMS");
...
void A76XXComponent::loop() {
  // Read message
  while (this->available()) {
    uint8_t byte;
    this->read_byte(&byte);

    if (this->read_pos_ == A76XX_READ_BUFFER_LENGTH)
      this->read_pos_ = 0;
...
void A76XXComponent::send_sms(const std::string &recipient, const std::string &message) {
  this->recipient_ = recipient;
  this->outgoing_message_ = message;
  this->send_pending_ = true;
}

void A76XXComponent::send_ussd(const std::string &ussd_code) {
...
void A76XXComponent::dial(const std::string &recipient) {
  this->recipient_ = recipient;
  this->dial_pending_ = true;
}
void A76XXComponent::connect() { this->connect_pending_ = true; }
void A76XXComponent::disconnect() { this->disconnect_pending_ = true; }

void A76XXComponent::parse_hex_str(std::string &str) {
  std::wstring_convert<std::codecvt_utf8_utf16<wchar_t>,wchar_t> conversion;
  std::wstring incoming;

  for (int ii = 0; ii < str.size(); ii += 4) {
    incoming += (wchar_t)std::strtoul(str.substr(ii, 4).c_str(), nullptr, 16);
  }

  str.clear();

  str = conversion.to_bytes(incoming.c_str());
}

void A76XXComponent::parse_sms() {
  parse_hex_str(this->message_);
  parse_hex_str(this->sender_);
}

void A76XXComponent::set_registered_(bool registered) {
